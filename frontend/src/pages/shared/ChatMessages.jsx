...
const ChatMessages = () => {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      try {
        ...
        // Agar PropertyDetails se chat aayi hai
        if (location.state?.chat?._id) {
          const existingChat = fetchedConversations.find(
            (chat) => chat._id === location.state.chat._id,
          );
        ...
        // Agar Home se Chat page open hua hai
        // aur koi active chat already hai
  ...
  }, [token, location.state, activeChat?._id, setActiveChat]);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        // FIXED: setMessages instead of setMessage
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Message failed, put text back in input
  ...
  const handleDeleteMessage = async (chatId, messageId) => {
    ...
    try {
      ...
      //   setMessages(res.data.chat.message);
      // } else if (res.data?.chat?.message) {
      //   setMessages(res.data.chat.message);
      // } else {
      //   setMessages((prev) => prev.filter((msg) => msg._id !== messageId));
      // }
      if (res.data?.chat?.message) {
        setMessages(res.data.chat.message);
      } else if (res.data?.chat?.message) {
        setMessages(res.data.chat.message);
      } else {
  ...
  const getChatPartner = (chat) => {
    ...
    const buyerId =
      typeof chat.buyer === "object" ? chat.buyer?._id : chat.buyer;
                    ...
                    return (
                      <div
                        ...
                        className={`${s.messageBubble} ${
                          isOwnMessage ? s.messageOwn : s.messageOther
                        }`}
                      >
                        <div className={s.messageContent}>
                          {/* Image */}
