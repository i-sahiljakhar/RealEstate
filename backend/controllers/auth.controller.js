...
export const register = async (req, res) => {
  try {
    ...
    try {
      await sendEmail({
        email,
        subject: "Verify Your Email - Real Estate Platform by Sahil jakhar",
        message: `<p>Your email verification code is : <strong>${verificationToken}</strong></p><p>Please enter this code on the verification page to activate to account where</p>`,
      });
    } catch (emailError) {
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const message = `
        ...
        <p>You required a password reset. Please click on the link below to reset you password:</p> <a href="${resetUrl}"clicktracking="off">${resetUrl}</a>
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Password  reset email send", success: true });
    } catch (error) {
