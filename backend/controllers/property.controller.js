...
//   try{
//     const existing = JSON.parse(req.body.existingImage);
//     property.image= Array.isArray(existing)? existing : property.image;

//   }catch(e){
//     console.error("Failed to parse  exisitngimage",e);
...
//   let newImage = [];
//   for(let file of req.files){
//     const result = await uploadingToCloudinary(file.buffer,"properties");
//     newImage.push(result.secure_url);
//   }
//   property.image = [...property.images, ...newImage];
// }
...
export const getPropertyDetails = async (req, res) => {
  try {
    ...
    // Increment property view
    const authHeader = req.headers.authorization;

    let isSellerChecking = false;
    ...
    if (authHeader && authHeader.startsWith("Bearer")) {
      try {
        ...
        isSellerChecking = decoded.id === property.seller._id.toString();
      } catch (err) {
        // ignore
      }
    }

    // Seller ki apni property ka view count nahi hoga
    if (!isSellerChecking) {
...
export const getSellerDashboard = async (req, res) => {
  try {
    ...
    //calculate total view for all properties
...
export const getPropertyCounts = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Internal server error while fetching properties counts",
    });
  }
};
